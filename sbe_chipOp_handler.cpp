...
static constexpr auto LENGTH_OF_RESP_HEADER_IN_WORDS = 0x2;
static constexpr auto MAX_FFDC_LEN_IN_WORDS = 5120;
                                    ...
                                    const size_t respBufLen)
{
    ...
    do
    {
        ...
        rc = write(fd, cmdBuffer, (cmdBufLen * 4));
        if (rc < 0)
        {
            //TODO:use elog infrastructure
        ...
        rc = read(fd, buffer.data(), (totalReadLen * 4));
        if (rc < 0)
        {
            //TODO:use elog infrastructure
